package module1.java.collection.map;
...
import module1.java.collection.list.Person;
...
class HashMapExamples {
  public static  void runExamples() {
     ...
     Person p1 = new Person(1, "one");
 	 ...
 	 HashMap<Person, Integer > pers= new HashMap<>();
 	 pers.put(p1,21);
 	 pers.put(p2,26);
 	 pers.put(p1,21);
 	 pers.put(p2,26);
 	...
 	int pp2=pers.get(p2);
 	int pp3=pers.get(p3);
 	System.out.println(pp+" "+pp2+" "+pp3);
 	 System.out.println(pers);
 	 
 	for (Entry<Person, Integer> entry : pers.entrySet()) {
 		Person person=entry.getKey();
        System.out.println(person.toString() + " -> " + entry.getValue());
    }
 	System.out.println("Map size after override equal and hashCode :"+pers.size());
